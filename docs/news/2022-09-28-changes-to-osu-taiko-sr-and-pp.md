...
We will also be using terminology coined by the osu!taiko community to refer to how patterns are placed. **d** refers to don (red) notes, and **k** refers to kat (blue) notes. Repeated notes of the same colour are referred to as _monos_ (short for monocolour).
...
Star rating is now based on _peak_ "strain" values incorporating all three skills covered by the algorithm, namely stamina and colour (as well as rhythm in the future). This is a significant change from the previous system, where only strain and individual skill levels were utilised instead.

_Peak_ difficulty splits the map into sections and combines the highest strains of each skill in each section, aiming to catch out particularly demanding moments. The final star rating is a weighted sum of the peak skill sections, including a slight multiplier to scale better.
