...
While the updates we've just made are not designed to fix _every_ issue with the systems as they currently stand, we hope that the changes will allow players to better assess skills that were not factored in properly in previous versions of the algorithm.
...
Note that all _new_ scores which are set will already be using the up-to-date performance algorithm.
